const PROMO_VIDEO_SRC = '/media/ld-event-design-promo.mp4';
...
function createPromoSection(): HTMLElement {
  ...
  ctaArrow.className = 'ld-promo-cta-arrow';
...
function syncPromoLanguage(section: HTMLElement) {
  ...
  const cta = section.querySelector('[data-promo-copy="cta"]');
  const arrow = cta?.querySelector('.ld-promo-cta-arrow');
  if (cta) {
    cta.replaceChildren(document.createTextNode(copy.cta), ...(arrow ? [arrow] : []));
  }
